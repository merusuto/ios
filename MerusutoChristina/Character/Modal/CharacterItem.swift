...
class CharacterItem {
    ...
    init(data: JSON) {
        ...
        country = getStringValue(data: data, key: "country")
        gender = data["gender"].intValue
        age = data["age"].intValue
        career = getStringValue(data: data, key: "career")
        interest = getStringValue(data: data, key: "interest")
        nature = getStringValue(data: data, key: "nature")
        // =====
...
//            }
    ...
    var rareString: String {
        get {
            return 0 < rare && rare < 6 ? String(repeating: "★", count: rare) : "暂缺"
        }
    }
    ...
    var elementString: String {
        get {
            return safeGetString(array: ["火", "水", "风", "光", "暗"], index: element)
        }
    }

    var typeString: String {
        get {
            return safeGetString(array: ["早熟", "平均", "晚成"], index: type)
        }
    }
    ...
    var genderString: String {
        get {
            return safeGetString(array: ["不明", "男", "女"], index: gender)
        }
    }
    ...
    // 零觉满级
    func calcMaxLv(_ value: Int) -> Int {
        return Int(Float(value) * calcF())
    }

    // 满觉满级
    func calcMaxLvAndGrow(_ value: Int) -> Int {
        let f = calcF()
